...
- `/session-init` now creates `todo/tasks/` and `todo/SEQUENCE.md`, optionally scaffolds a `PRD.md` direction stub, and wires an idempotent `<!-- session-flow:sequence -->` block into the project's `CLAUDE.md` and `AGENTS.md` so "implement the next task" works without naming a file.
- `.session-flow.json` gains `paths.tasks`, `paths.sequence`, and `paths.direction` (default `PRD.md`, overridable to an existing direction file).
