...
session-init (step 0)
    |
    +--> creates SEQUENCE.md + todo/tasks/ + (optional) PRD.md
    |    and wires the sequence block into CLAUDE.md / AGENTS.md
    |
    v
session-gatekeeper --> triage incoming work
    |
    v
...
session-task-planning --> todo/
    |
    v
session-add-task / session-groom --> SEQUENCE.md (backlog)
    |
    v
session-next / session-delegation --> executes tasks
    |
    v
