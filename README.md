...
That qualifier is load-bearing now that `/scribe code` also writes to `SEQUENCE.md`. A capture has no outside item behind it — it is a thought typed into the terminal — so it is filed unannotated on purpose, and the annotation is written later by whichever mirror first files it outward. An unannotated entry means *not yet mirrored*, never *not yet checked for duplicates*. Both plugins allocate `SEQ-NNN` from the highest id already in the file; count `[DEFERRED]` entries when you do, since they hold retired ids that no active-only scan will see. That allocation is lock-free — what makes it safe is discipline, not mechanism: at most one scheduled writer per repo, and pull before any local edit to the sequence, so a concurrent CI enqueue and a local import never mint the same id.
